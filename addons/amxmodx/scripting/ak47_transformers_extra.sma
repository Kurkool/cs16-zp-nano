...
#define IDLE_TIME		2.4
#define DRAW_TIME		1.02
#define RELOAD_TIME		2.93
...
#define WEAPON_KEY				51415444783563
#define Is_CustomItem(%0)			(pev(%0,pev_impulse)==WEAPON_KEY)

#define model_v		"models/addons/v_ak47_transformers.mdl"
#define model_p		"models/addons/p_ak47_transformers.mdl"
#define model_w		"models/addons/w_ak47_transformers.mdl"

#define weapon_punchangle		0.5	
#define weapon_damage			1.5					
#define weapon_aspeed			0.1			
#define weapon_max_energy               100.0
#define weapon_energy_restore           0.0

#define weapon_ammo	30
#define weapon_bpammo	90
#define sound_shot	"addons/ak47_transformers_fire1.wav"

#define shock_damage    700.0
#define shock_radius    80.0

new g_AllocString_V,g_AllocString_P
new g_Shock
new Float:g_VecEndTrace[3];
new g_item,g_Energy[512],gMaxPlayers,g_Id,g_Ent
...
public plugin_precache(){
	...
	g_Shock=engfunc(EngFunc_PrecacheModel,"sprites/addons/blaster_explosion1.spr")
	
	engfunc(EngFunc_PrecacheSound,sound_shot)
	
	g_item=zp_register_extra_item("AK-47 Transformers",2,ZP_TEAM_HUMAN);
...
public HookHam_Weapon_Frame(ent){
	...
	client_print(id,print_center,"[Energy:%d]",g_Energy[ent])
	...
	if(get_pdata_float(ent,pDataKey_flNextSecondaryAttack,4)<=0.0||get_pdata_float(ent,pDataKey_flNextTimeWeaponIdle,4)<=IDLE_TIME){
		if(pev(id,pev_button)&IN_ATTACK2){
			if(g_Energy[ent]==weapon_max_energy){
				...
				new Float:Origin[3]
				get_weapon_position(id,Origin,.add_forward=10.0,.add_right=11.0,.add_up=-10.5)
...
public HookFm_TraceLine(Float:tr_start[3],Float:tr_end[3],tr_flag,tr_ignore,tr){
	...
	if(g_Energy[g_Ent]==weapon_max_energy && pev(g_Id,pev_button)&IN_ATTACK2){
		engfunc(EngFunc_MessageBegin,MSG_PVS,SVC_TEMPENTITY,vecEnd,0);
		...
		for(new i=1;i<=gMaxPlayers;i++){ 
			if(is_user_alive(i)){
				...
				if(i!=g_Id&&get_user_team(i)!=get_user_team(g_Id)&&Distance<=shock_radius){ 
					emit_sound(i,CHAN_AUTO,"addons/blaster_explosion1.wav",VOL_NORM,ATTN_NORM,0,PITCH_NORM)
