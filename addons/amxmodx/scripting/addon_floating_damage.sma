 ...
 */
//#include <reapi>   // no reapi module on this server
...
public CDamager__SpawnEntity( const pPlayer, const pVictim, Vector3( vecOrigin ), Float: flDamage, const bool: bHeadShot )
{
	...
	if ( pEntity != NULLENT )
	{
		...
		/*
			Per hit, not a running total.

			Shipped behaviour was to add this hit onto whatever the reused
			entity was already showing, so holding the trigger turned the
			number into a counter that climbed until the victim died. The
			accumulate lines are gone; the hit is stored on its own, so each
			shot replaces the last instead of adding to it.

			Everything below still resets the animation, the alpha and the
			think timer, so the number restarts cleanly on every shot rather
			than finishing the previous one's fade.

			To put the original behaviour back, read var_dmg_save here and
			add it to flDamage before this line.
		*/
