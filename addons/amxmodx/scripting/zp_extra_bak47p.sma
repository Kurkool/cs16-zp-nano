...
new cvar_dmg, cvar_recoil, cvar_clip, cvar_spd, cvar_ammo, cvar_radius, cvar_dmg_2, cvar_trace_color, g_bak47p
...
public plugin_init()
{
	...
	g_bak47p = zp_register_extra_item("bak47p", 45, ZP_TEAM_HUMAN)
}
...
public native_remove_bak47p(id)
{
	remove_bak47p(id)
}
...
public client_disconnect(id) remove_bak47p(id)
public zp_user_infected_post(id) remove_bak47p(id)   // restored by setup: infection = you lose the weapon (ZP strips it anyway). Still kept across humanize/death/respawn/round.
public zp_user_humanized_post(id) remove_bitvar(id)   // unified by setup: ownership survives infect/humanize/death/respawn/round; cleared only on connect, disconnect, drop
...
public Forward_PlayerKilled(id) remove_bitvar(id)   // unified by setup: ownership survives infect/humanize/death/respawn/round; cleared only on connect, disconnect, drop
