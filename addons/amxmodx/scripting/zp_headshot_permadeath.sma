/*
	...
	3. Endless respawn - non-melee deaths respawn after zp_endless_respawn_delay
	...
	cvars:
		zp_hs_permadeath           1 = melee kills stick for the round
		                           (name kept so the existing cfg still works)
		zp_hs_permadeath_announce  1 = chat line on a permanent kill
		zp_kill_sound              1 = play the kill feedback sound
		zp_endless_respawn         1 = take respawn over from ZP
		zp_endless_respawn_delay   seconds before a normal death comes back
		zp_escape_kill_credit      1 = frag + kill feed when the last zombie escapes
*/
...
public plugin_init()
{
	...
	g_pEnabled   = register_cvar("zp_hs_permadeath", "1")
	g_pAnnounce  = register_cvar("zp_hs_permadeath_announce", "1")
	g_pKillSound = register_cvar("zp_kill_sound", "1")
	g_pRespawn   = register_cvar("zp_endless_respawn", "1")
	g_pCredit    = register_cvar("zp_escape_kill_credit", "1")
	...
	*/
	g_pRespawnDelay = register_cvar("zp_endless_respawn_delay", "1.0")
...
public Fw_TakeDamage_Pre(victim, inflictor, attacker, Float:damage, damagebits)
{
	...
	if (get_pcvar_num(g_pDebug))
		log_amx("[PERMA] lethal victim=%d attacker=%d dmg=%.0f hp=%.0f melee=%d bits=%d humanAtk=%d zombiesAlive=%d",
	...
	{
		if (get_pcvar_num(g_pDebug))
			log_amx("[PERMA]   -> allowed to die as a PERMAKILL")
	...
	if (get_pcvar_num(g_pDebug))
		log_amx("[PERMA]   -> LAST ZOMBIE rescued, damage cancelled, healed in place")
...
public Fw_Killed_Post(victim, attacker, shouldgib)
{
	...
	{
		if (get_pcvar_num(g_pDebug))
			log_amx("[PERMA] killed_post victim=%d permadead=1 -> NO respawn, stays down", victim)
	...
	if (get_pcvar_num(g_pDebug))
		log_amx("[PERMA] killed_post victim=%d permadead=0 -> respawn in %.2fs", victim, delay)
...
public Task_Respawn(taskid)
{
	...
	{
		if (get_pcvar_num(g_pDebug))
			log_amx("[PERMA] respawn_task victim=%d SKIPPED connected=%d alive=%d permadead=%d",
	...
	if (get_pcvar_num(g_pDebug))
		log_amx("[PERMA] respawn_task victim=%d -> respawning as zombie", id)
...
public Event_DeathMsg()
{
	...
	if (get_pcvar_num(g_pDebug))
		log_amx("[PERMA] deathmsg victim=%d killer=%d meleeFlag=%d csHeadshotFlag=%d -> permakill=%d",
